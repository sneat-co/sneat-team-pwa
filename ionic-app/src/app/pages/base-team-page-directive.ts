import {IRecord, ITeam, IUserTeamInfoWithId} from '../models/interfaces';
import {ActivatedRoute} from '@angular/router';
import {ErrorLogger, IErrorLogger} from '@sneat-team/ui-core';
import {NavController} from '@ionic/angular';
import {TeamService} from '../services/team.service';
import {ChangeDetectorRef, Directive, Inject, Injectable, OnDestroy} from '@angular/core';
...
export abstract class BaseTeamPageDirective implements OnDestroy {
	...
	protected trackTeamIdFromUrl(teamParamName: string = 'team'): void {
		try {
			this.teamContextService.trackUrl(this.route, teamParamName)
				.pipe(takeUntil(this.destroyed))
				.subscribe({
					next: id => {
						console.log(`BaseTeamPageDirective.trackTeamIdFromUrl() => ${teamParamName}:`, id);
						this.setTeamId(id);
					},
					error: err => this.logError(err, 'Failed to track team ID from url'),
				});
		} catch (e) {
	...
	protected onTeamChanged(): void {
		...
		this.userTeam = this.teamRecord ? {id: this.team?.id, title: this.team.data.title} : undefined;
	}
					...
					this.subs.add(
						this.userService.userChanged.pipe(
							...
							mergeMap(() => this.teamService.watchTeam(id)),
								...
								error: err => this.logError(err, `failed to load team by id=${id}`),
							},
						),
					);
				}
