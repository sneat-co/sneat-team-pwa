...
import {ITimerState} from '../models/interfaces';
...
export class Timer {
	...
	public updateTimerState(timerState?: ITimerState): ITimerState {
		...
		try {
			...
			const prevAt = prevState && typeof prevState.at === 'string'
				? Timestamp.fromDate(new Date(prevState.at))
				: prevState?.at as Timestamp;
			const {elapsedSeconds} = timerState;
			const at = typeof timerState.at === 'string' ? Timestamp.fromDate(new Date(timerState.at)) : timerState.at
			if (prevAt?.nanoseconds !== at.nanoseconds) {
				this.state = {
					...timerState,
					isToggling: this.state?.isToggling,
					status: this.state?.isToggling ? this.state?.status : timerState.status,
					elapsedSeconds: timerState.status === TimerStatusEnum.active
						? (elapsedSeconds || 0) + (new Date().getTime() / 1000 - at.seconds)
			...
			this.errorLogger.logError(e, 'Failed to update timer state', {feedback: false, show: false});
		}
		return this.state;
	}
	...
	public toggleTimer(member?: string): Observable<never> {
		...
		switch (this.state?.status) {
			...
			case TimerStatusEnum.paused:
				...
				break;
			case TimerStatusEnum.active:
				operation = !member || member === this.state.activeMemberId ? TimerOperationEnum.stop : TimerOperationEnum.start;
				break;
			default:
	...
	public setTimerState(operation: TimerOperation, member?: string): Observable<never> {
		...
		try {
			...
			this.state = {
				...this.state,
				isToggling: true,
				status: operation === TimerOperationEnum.start ? TimerStatusEnum.active
					: operation === TimerOperationEnum.stop ? TimerStatusEnum.stopped
						: undefined,
			...
			const toggleTimerApi = member ? this.timerService.toggleMemberTimer : this.timerService.toggleMeetingTimer;
				...
				.subscribe({
					next: response => {
						try {
							...
							this.errorLogger.logError('toggleTimer() => failed to process API response', e);
						}
					},
					error: err => {
						try {
							...
							this.errorLogger.logError('toggleTimer() => failed to process observable error', e);
						}
					},
				});
		} catch (e) {
			...
			try {
				...
				this.errorLogger.logError('toggleTimer() => failed to process error', ee);
			}
		}
	...
	private startTicking(): void {
		...
		this.unsubscribeInterval();
		this.emitTick();
		this.intervalSubscription = interval(1000).subscribe({next: this.nextSecond});
	}
	...
	private nextSecond = (): void => {
		...
		if (this.state.activeMemberId) {
			...
			this.state.secondsByMember = {
				...this.state.secondsByMember,
				[this.state.activeMemberId]: (this.state.secondsByMember[this.state.activeMemberId] ?? 0) + 1,
			};
		}
		this.emitTick();
	}
	...
	private emitTick(): void {
		const state = this.state, lastTick = this.lastTick;
		if (
			!lastTick
...
export class TimerFactory {
	...
	public getTimer(timerService: IMeetingTimerService, teamId: string, meetingId: string): Timer {
		if (!teamId) {
		...
		return this.timers[k] ?? (this.timers[k] = this.newTimer(k, timerService, teamId, meetingId));
	}

	private newTimer = (k: string, timerService: IMeetingTimerService, teamId: string, meetingId: string) =>
		new Timer(this.errorLogger, timerService, this.onTimerReleased(k), teamId, meetingId);
