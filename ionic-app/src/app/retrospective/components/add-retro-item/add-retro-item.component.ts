...
import {IAddRetroItemRequest, RetroItemType, RetrospectiveService} from '../../retrospective.service';
...
export class AddRetroItemComponent implements OnDestroy {
	...
	public add(event?: Event): void {
		...
		this.isAdding = true;
		this.retrospectiveService.addRetroItem(request)
			.pipe(takeUntil(this.destroyed))
			.subscribe({
				next: response => {
					console.log('retrospectiveService.addRetroItem() => response:', response);
					this.isAdding = false;
				},
				error: err => {
